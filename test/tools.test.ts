...
test('formatResult truncates oversized payloads and keeps structuredContent for objects', () => {
  const big = formatResult({ items: 'x'.repeat(CHARACTER_LIMIT + 10) });
  assert.match(big.content[0].type === 'text' ? big.content[0].text : '', /Truncated/);
